...
export class Thread extends React.Component<ThreadProperties, ThreadVariables>{
	...
	getThreadJSONData(board:string, id:string){
		return new Promise((resolve, reject) => {
			...
			thread_req.addEventListener("load", function(re:any){
				if(this.status >= 300){
					reject(this.status);
				}
				else{
			...
			thread_req.addEventListener("error", function(err:any){
				reject(this.status);
			})
    ...
    this.state = {spaced_posts:spaced_list, error:null, expanded: false};
	}

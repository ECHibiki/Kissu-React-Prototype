...
export class Post extends React.Component<PostProperties, PostDetails>{
	...
	constructor(props:any){
		...
		this.state = ({filename_cutoff:20, file_details_hidden:false, show_full_image: false, show_full_audio:false, show_full_video:false,
...
// JQUERY types local has been added but should be removed
	...
	createImageSearchLink(search_engine_name:string, search_engine_pattern:string):JSX.Element{
		var source = "/" + this.props.board + "/src/" + this.props.tim + this.props.ext;
		...
		for(var cite of cite_arr){
			if(cite['board'] ==  this.props.board){
				return_jsx.push(<span key={i++}>&nbsp;
						  <a href={ "/" + this.props.board + "/res/" + cite['host'] + "#" + cite['post'] }>{ ">>"  + cite['post']}</a>
						</span>)
			}
			else{
				return_jsx.push(<span key={i++}>&nbsp;<a href={ "/" + cite['board'] + "/res/" + cite['host'] + "#" + cite['post'] } >{ ">>>/" + cite['board'] + "/" + cite['post']}</a></span>)
			}
...
// NOTE: data-op is a temporary messure to maintain compatibility with legacy JS
		...
		return (<div data-op={this.props.op_id} id={this.props.hierarchy_class + "_" + this.props.id} className={"post " + this.props.hierarchy_class + " " + (this.props.highlighted ? "highlighted" : "")}>
